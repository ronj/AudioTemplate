...
/* $Id: mpadec.c,v 1.4 2011/12/26 17:53:30 jpff Exp $ */
...
{
  struct mpadec_t *mpa = (struct mpadec_t *)malloc(sizeof(struct mpadec_t));
  union {
      uint8_t  t8[2];
      uint16_t t16;
  } ch;
  ...
  mpa->config.endian = (uint8_t)(ch.t8[0] ? MPADEC_CONFIG_LITTLE_ENDIAN : MPADEC_CONFIG_BIG_ENDIAN);
...
int mpadec_get_info(mpadec_t mpadec, void *info, int info_type)
{
  ...
  if (info_type == MPADEC_INFO_CONFIG) {
    mpadec_config_t *cfg = (mpadec_config_t *)info;
    ...
    mp3tag_info_t *tag = (mp3tag_info_t *)info;
    ...
    mpadec_info_t *inf = (mpadec_info_t *)info;
    if (mpa->state < MPADEC_STATE_DECODE) {
...
{
  struct mpadec2_t *mpa = (struct mpadec2_t *)malloc(sizeof(struct mpadec2_t));
...
{
  struct mpadec2_t *mpa = (struct mpadec2_t *)mpadec;
