...
static int decode_layer3_sideinfo(mpadec_t mpadec)
{
  ...
  for (gr = 0; gr < tab[0]; gr++) {
    for (ch = 0; ch < mpa->frame.channels; ch++) {
      grinfo_t *grinfo = &mpa->sideinfo.ch[ch].gr[gr];
...
static void III_i_stereo(mpadec_t mpadec, grinfo_t *gr_info,
                         FLOAT xrbuf[2][SBLIMIT][SSLIMIT], int32_t *scalefac)
{
  struct mpadec_t *mpa = (struct mpadec_t *)mpadec;
  grinfo_t *grinfo = gr_info;
...
static void dct36(FLOAT *in, FLOAT *out1, FLOAT *out2, FLOAT *w, FLOAT *ts)
{
  FLOAT tmp[18];
...
static void dct12(FLOAT *in, FLOAT *out1, FLOAT *out2, FLOAT *w, FLOAT *ts)
{
...
static void III_hybrid(mpadec_t mpadec, grinfo_t *gr_info, FLOAT fs_in[SBLIMIT][SSLIMIT], FLOAT ts_out[SSLIMIT][SBLIMIT], int channel)
{
  ...
  {
    unsigned b = mpa->hybrid_block[channel];
